...
# ---------------------------------------------------------------------------- #
#                                                                              #
#            Model code to run random walk with covariate (W-F Plover)         #
#                                   Forecast                                   #
#                                                                              #
# ---------------------------------------------------------------------------- #
# ---------------------------------------------------------------------------- #
...
# # Counts
# genus <- "Charadrius"
# sp_epithet <- "marginatus"
# sp_counts_plovers <- bird_counts %>% 
#   filter(Genus == genus & Species == sp_epithet)
# 
# sp_counts_plovers_filtered <- sp_counts_plovers %>% 
#   select(StartDate, TimeStart, TimeEnd, Count, Pairs, X, Y)
# 
# write.csv(sp_counts_plovers_filtered, 
#           "./project_data/sp_counts_plovers_filtered.csv")
# 
# sp_counts_plovers_filtered %>%
...
# Covariates
covs <- read.csv("./project_data/new_env_plovers.csv")

plover <- read.csv("C:/Users/Vulpe/Documents/GitHub/BIRDIE/project_data/sp_counts_plovers_filtered.csv")
plover$StartDate <- as.POSIXct(plover$StartDate, 
                               format = "%Y-%m-%d")
plover$Date <- format(plover$StartDate, 
                      "%Y-%m")

plover <- plover %>% 
  filter(Date %in% covs$Date)

time = as.Date(plover$StartDate)

plover[135:155,5:6] <- NA
